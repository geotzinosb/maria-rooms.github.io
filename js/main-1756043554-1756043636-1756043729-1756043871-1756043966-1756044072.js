...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initMobileMenu() {
        if (mobileMenu) {
            ...
            // Force the hidden state with inline styles for extra reliability
    ...
    // Add mutation observer to ensure menu state consistency
    if (mobileMenu) {
        const observer = new MutationObserver((mutations) => {
            mutations.forEach((mutation) => {
                if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
                    const hasHidden = mobileMenu.classList.contains('hidden');
                    const computedDisplay = window.getComputedStyle(mobileMenu).display;
                    
                    // If hidden class is present but display is not none, fix it
                    if (hasHidden && computedDisplay !== 'none') {
                        console.log('Fixing inconsistent menu state - hidden class but visible display');
                        mobileMenu.style.display = 'none';
                        mobileMenu.style.visibility = 'hidden';
                        mobileMenu.style.opacity = '0';
                        mobileMenu.style.pointerEvents = 'none';
                    ...
                    // If hidden class is not present but display is none, fix it
                    if (!hasHidden && computedDisplay === 'none') {
                        console.log('Fixing inconsistent menu state - no hidden class but hidden display');
        ...
        console.log('Mobile menu mutation observer initialized');
    }
    
    // Function to ensure menu is properly hidden
        ...
        if (mobileMenu) {
            ...
            setTimeout(() => {
                if (!mobileMenu.classList.contains('hidden')) {
                    console.log('Menu missing hidden class, forcing addition');
                    mobileMenu.classList.add('hidden');
                }
                console.log('Menu hidden state:', {
                    classList: mobileMenu.className,
                    display: mobileMenu.style.display,
                    visibility: mobileMenu.style.visibility,
    ...
    // Function to ensure menu is properly visible
    function ensureMenuVisible() {
        if (mobileMenu) {
            ...
            setTimeout(() => {
                if (mobileMenu.classList.contains('hidden')) {
                    console.log('Menu still has hidden class, forcing removal');
                    mobileMenu.classList.remove('hidden');
                }
                console.log('Menu visibility state:', {
                    classList: mobileMenu.className,
                    display: mobileMenu.style.display,
                    visibility: mobileMenu.style.visibility,
                    computedDisplay: window.getComputedStyle(mobileMenu).display
    ...
    if (mobileMenuButton && mobileMenu) {
        ...
        const openMenu = () => {
            ...
            setTimeout(() => {
                ...
                console.log('Mobile menu inline styles:', mobileMenu.style.display, mobileMenu.style.visibility);
            }, 100);
        };
        ...
        // Add touchstart event for better mobile compatibility
        mobileMenuButton.addEventListener('touchstart', openMenu);
        
        // Add mousedown event as fallback
        mobileMenuButton.addEventListener('mousedown', openMenu);
    }

    if (mobileMenuClose && mobileMenu) {
        mobileMenuClose.addEventListener('click', function() {
            console.log('Mobile menu close button clicked, closing menu');
            ensureMenuHidden();
            document.body.style.overflow = 'auto';
        });
    }
