...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initMobileMenu() {
        if (mobileMenu) {
            // Always start with menu hidden using hidden class
            mobileMenu.classList.add('hidden');
    ...
    // Simple functions to show/hide menu
    ...
    if (mobileMenuButton && mobileMenu) {
        ...
        const openMenu = () => {
            ...
            setTimeout(() => {
                ...
                const zIndex = mobileMenu.style.zIndex;
                ...
                // If menu is still hidden, force it open
                if (!isVisible) {
                    console.log('Menu still hidden, forcing open');
                    showMobileMenu();
                }
            }, 100);
        };
...
function initializeLanguageSwitcher() {
    ...
    const desktopLanguageOptions = document.querySelectorAll('.language-option');
...
function switchLanguage(lang) {
    ...
    // If we're already on a language-specific path, extract the base path
    let basePath = currentPath;
    if (currentPath.startsWith('/en/') || currentPath.startsWith('/el/') || 
        currentPath.startsWith('/de/') || currentPath.startsWith('/bg/')) {
        // Remove the language prefix to get the base path
        basePath = currentPath.substring(3); // Remove '/en/', '/el/', etc.
    }
    
    // If it's the root path, just add the language
    ...
    let newUrl;
    if (lang === 'en') {
        // English is the default language, so no prefix needed
        newUrl = basePath;
    } else {
